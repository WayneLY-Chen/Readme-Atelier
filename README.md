...
*[English](#readme-atelier-english) below.*
...
**1.** 在你 profile repo 的根目錄放一份 `widgets.yml`：
...
可以直接複製的範例檔在 [`examples/consumer/`](examples/consumer/)。
...
`language` 只讀最上層——不接受卡片層級覆寫，也不會從你的帳號自動推論。

`id` 決定輸出檔名（`<id>-light.svg` / `<id>-dark.svg`），所以同一個 `type` 用兩次時，要給其中一張
明確的 `id`：
...
`widgets.yml` 寫錯會讓整次執行失敗，並**一次列出所有問題**，附行號、原始行與修正建議——絕不會默默
忽略它看不懂的鍵。
...
- **文字全部轉成路徑。** 以圖片形式載入的 SVG 無法載入字型，因此字符在渲染時就嵌成路徑資料，
  卡片在任何地方看起來都一樣。
- **無障礙。** SVG 以圖片被引用時，只有 `<img alt>` 會傳達給輔助科技（寫在 SVG 裡的 `<title>`
  永遠不會），所以產生的片段帶有真正描述卡片內容的 alt 文字，貼上時請保留。
...
npm run preview        # 依 ./widgets.yml（或預設值）渲染到 .preview/
npm test
npm run build          # 打包 action 到 dist/
```

`src/core/**` 與 `src/widgets/**` 刻意完全不碰檔案系統——所有磁碟 I/O 只存在於 `src/node/` 與兩個
進入點（`src/cli.ts`、`src/action-entry.ts`）。兩個進入點都經由同一支 `renderAllCards()` 渲染，
所以卡片在本機與在 CI 不可能長得不一樣。
...
page-load time, and there is no service that can go down and take your README with it.
...
`language` is read from the top level only — never set per-card, never guessed from your profile.
`id` decides the output filenames (`<id>-light.svg` / `<id>-dark.svg`), so give a card an explicit
`id` when you use the same `type` twice.
...
- **Text is converted to paths.** An SVG loaded as an image cannot load fonts, so glyphs are
  embedded as path data at render time.
...
in `src/node/` and the two entry points. Both entry points render through the same
`renderAllCards()`, so a card cannot look different locally than it does in CI.
